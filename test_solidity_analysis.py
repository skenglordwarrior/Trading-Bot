...
class LiquidityLockDetectionTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_holder_shortcut_skips_heuristic(self):
        holder_mock = AsyncMock(return_value=True)
        heuristic_mock = AsyncMock(return_value=False)

        with patch.object(
            ethereumbotv2, "_check_liquidity_locked_holder_analysis", holder_mock
        ), patch.object(
            ethereumbotv2, "_check_liquidity_locked_heuristic_async", heuristic_mock
        ):
            locked = await ethereumbotv2._check_liquidity_locked_etherscan_async("0xPAIr")
