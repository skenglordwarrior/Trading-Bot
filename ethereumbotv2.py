...
def _lock_details_need_enrichment(
    details: Optional[LiquidityLockDetails],
) -> bool:
    """Return True when lock metadata lacks the timestamps we need for alerts."""

    if not details:
        return True
    if details.locked_at and details.unlock_at:
        return False
    return True


def _merge_lock_details(
    primary: Optional[LiquidityLockDetails],
    secondary: Optional[LiquidityLockDetails],
) -> Optional[LiquidityLockDetails]:
    """Combine two lock snapshots, preferring coverage from the primary source."""
    ...
    return LiquidityLockDetails(
        source=primary.source or secondary.source,
        coverage_pct=(
            primary.coverage_pct
            if primary.coverage_pct is not None
            else secondary.coverage_pct
        ),
        locked_at=primary.locked_at or secondary.locked_at,
        unlock_at=primary.unlock_at or secondary.unlock_at,
    )
...
async def _check_liquidity_locked_etherscan_async(
    ...
    if holder_based is True:
        if _lock_details_need_enrichment(holder_details):
            uncx_based, uncx_details = await _check_liquidity_locked_uncx_async(pair_addr)
            if uncx_based is True:
                holder_details = _merge_lock_details(holder_details, uncx_details)
        return True, holder_details

    if holder_based is False:
        return False, holder_details
...
def _build_lock_info_line(
    ...
    if not locked_at and created_at:
    ...
    if (
        ...
        and created_at
        and unlock_at
        and unlock_at > created_at
    ):
        duration_value = unlock_at - created_at
