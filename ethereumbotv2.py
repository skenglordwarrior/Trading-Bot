...
async def _check_liquidity_locked_heuristic_async(pair_addr: str) -> bool:
...
async def _check_liquidity_locked_etherscan_async(pair_addr: str) -> bool:
    """Return True when liquidity appears locked, False otherwise.

    The holder-distribution analysis delivers authoritative verdicts when it can
    classify enough LP supply. When it cannot, we fall back to the historical
    Etherscan-transfer heuristic to preserve prior behaviour.
    """
    ...
    return await _check_liquidity_locked_heuristic_async(pair_addr)
